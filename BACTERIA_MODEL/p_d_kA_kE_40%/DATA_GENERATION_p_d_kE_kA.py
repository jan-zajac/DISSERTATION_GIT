# data generation code that varies only in terms of p, d, kE and kA and has four targets by 40%
...
for i in range(tot_data):
    ...
    kA = round(random.uniform(0.012, 0.028), 4) # Natural degradation rate of intracellular 3-oxo-C12-HSL (autoinducer)
    kE = round(random.uniform(0.06, 0.14), 4) # Natural degradation rate of extracellular 3-oxo-C12-HSL (autoinducer)
    ...
    # appending empty list with concentration trajectory solutions
    R_sols.append(R)
    A_sols.append(A)
...
# save data to files
#np.save('0_p_d_ODE_sols_3D_4dp_100000', sols)
#np.savetxt('0_p_d_kA_kE_ODE_sols_TEST_2000.txt', A_sols, delimiter= ',' )
#np.savetxt('0_p_d_kA_kE_targets_TEST_2000.txt', targets, delimiter= ',')